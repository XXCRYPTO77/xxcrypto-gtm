...
export function TransitionBand({ band, nextActLabel, nextActHref }: TransitionBandProps) {
  ...
  return (
    <section className="relative border-y border-border bg-surface py-10 px-6">
      <div className="mx-auto max-w-7xl">
        {/* Version transition header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-8">
          <div className="flex items-center gap-3">
            <span className="rounded-full border border-border bg-page px-3 py-1 font-mono text-xs font-bold text-muted shadow-sm">
              {data.from}
            </span>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {data.items.map((item) => (
            <div
              key={item.code}
              className="flex items-start gap-3 rounded-xl border border-border bg-page p-4 shadow-sm"
            >
              <span className="shrink-0 rounded-md bg-surface px-2 py-0.5 font-mono text-xs font-bold text-muted">
                {item.code}
              </span>
